...
public class FollowerPIDTuner extends CommandOpMode {
    ...
    public static double TURN_DEGREE=90;  //90 degreefor square
    ...
    public void initialize() {
        ...
        schedule(new RunCommand(() -> {
            if (trajGroup == null || trajGroup.isFinished() || !trajGroup.isScheduled()) {
                ...
                turnCommand = new TurnCommand(drive, Math.toRadians(TURN_DEGREE));
                trajGroup = new SequentialCommandGroup(followerCommand, turnCommand)
                        .whenFinished(() -> startPose = traj.end().plus(new Pose2d(0, 0, Math.toRadians(TURN_DEGREE))));
                trajGroup.schedule();
            }
        }));
    }
